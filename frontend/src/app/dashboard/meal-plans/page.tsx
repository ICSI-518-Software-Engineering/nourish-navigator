  ...
  const handleLikeDislike = (meal: any, reaction: "like" | "dislike") => {
    ...
    setMealReactions((prevState) => ({
      ...
      [mealName]: {
        ...prevState[mealName],
        likeDislike:
          prevState[mealName]?.likeDislike === reaction ? undefined : reaction,
    ...
    <div>
      ...
      {currentMeals?.map?.((plan: any, planIndex: any) => (
        <React.Fragment key={planIndex}>
          ...
          <div
            className="rounded-xl border bg-card text-card-foreground shadow dark:border-gold-900 p-16"
            style={{
              display: "grid",
            ...
            {plan?.meal?.map?.((m: any, index: any) => (
              ...
              >
                ...
                <div className="flex justify-center items-center mt-3 space-x-2">
                  <button
                    onClick={() => handleLikeDislike(m, "like")}
                    className={`p-2 rounded-full text-white shadow-lg transform transition-transform duration-200 ${
                      mealReactions[m?.mealName]?.likeDislike === "like"
                  ...
                  <button
                    onClick={() => handleMarkConsumed(m)}
                    className={`p-2 rounded-full text-white shadow-lg transform transition-transform duration-200 ${
                      mealReactions[m?.mealName]?.consumed
                  ...
                  <button
                    onClick={() => handleLikeDislike(m, "dislike")}
                    className={`p-2 rounded-full text-white shadow-lg transform transition-transform duration-200 ${
                      mealReactions[m?.mealName]?.likeDislike === "dislike"
                  ...
                  <button
                    onClick={() => handleReject(m?.mealName)}
                    className={`p-2 rounded-full text-white shadow-lg transform transition-transform duration-200 ${
                      mealReactions[m?.mealName]?.rejected
                        ? "bg-red-500 hover:bg-red-600"
                        : "bg-red-200 hover:bg-red-300"
                    }`}
                  >
